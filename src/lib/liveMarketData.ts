...
export interface ABTestResult {
  success: boolean;
  mode: string;
  has_active_technical_rule_profile_b: boolean;
  assertion_passed: boolean;
  profile_a: {
    technical_rules_enabled: boolean;
    qualified_contracts: number;
  ...
  identical_contracts: number;
  different_contracts: number;
  diffs: ABTestDiff[];
  test_passed: boolean;
  scanned_at: string;
}
...
export async function runABTest(
  ...
  if (error) throw new Error(error.message || 'A/B test failed');
  if (!data || data.success === false) throw new Error(data?.error || 'A/B test returned failure');
